        ...
        <form @submit.prevent="update">
            <div class="grid grid-cols-6 gap-4">
                ...
                <div class="col-span-4">
                    ...
                    />
                    <InputError
                        v-show="form.errors.code"
                        :error="form.errors.code"
                    />
                </div>
                <div class="col-span-2">
                    <label class="label" for="street_no">Street-No</label>
                    <input
                        type="text"
                        name="street_no"
                    ...
                    />
                    <InputError
                        v-show="form.errors.street_no"
                        :error="form.errors.street_no"
                    />
                </div>
...
<script setup>
import { useForm } from "@inertiajs/vue3"; // useForm = { router.post('/store') } + reactive()

const props = defineProps({ list: Object });
const form = useForm({
    beds: props.list.beds,
    baths: props.list.baths,
    area: props.list.area,
    city: props.list.city,
    street: props.list.street,
    code: props.list.code,
    street_no: props.list.street_no,
    price: props.list.price,
});

const update = () =>
    form.put(route("realtor.listing.update", { listing: props.list.id }));
</script>
